...
""", },
	...
	public ActionRoom(
		...
		bool useVoxelWeapons = true,
		int? playerXOverride = null,
		int? playerYOverride = null,
		short? playerAngleOverride = null,
		StatusBarController statusBarController = null,
		StatusBarRenderer statusBarRenderer = null)
	{
	...
	public override void _Ready()
	{
		try
		{
			...
			_fixtures = new Fixtures(
				spriteMaterials: VRAssetManager.SpriteMaterials,
				staticSpawns: MapAnalysis.StaticSpawns,
				getCameraYRotation: () => _displayMode.ViewerYRotation);
			AddChild(_fixtures);
			...
			if (_existingSimulator is not null)
				...
				_simulatorController.InitializeFromExisting(
					existingSimulator: _existingSimulator,
					doorsNode: _doors,
					wallsNode: _walls,
					bonusesNode: _bonuses,
					actorsNode: _actors,
					weaponsNode: _weapons,
					projectilesNode: _projectiles,
					getPlayerPosition: () => (_displayMode.ViewerPosition.X.ToFixedPoint(), _displayMode.ViewerPosition.Z.ToFixedPoint(), _displayMode.ViewerYRotation.ToWolf3DAngle()));
			else
				// New game, level transition, or loading saved game - create fresh simulator
				_simulatorController.Initialize(
					mapAnalyzer: Shared.SharedAssetManager.CurrentGame.MapAnalyzer,
					...
					weaponsNode: _weapons,
					projectilesNode: _projectiles,
					stateCollection: Shared.SharedAssetManager.CurrentGame.StateCollection,
					weaponCollection: Shared.SharedAssetManager.CurrentGame.WeaponCollection,
					getPlayerPosition: () => (_displayMode.ViewerPosition.X.ToFixedPoint(), _displayMode.ViewerPosition.Z.ToFixedPoint(), _displayMode.ViewerYRotation.ToWolf3DAngle()),
					statusBar: SharedAssetManager.StatusBar,
					difficulty: _difficulty,
					savedInventory: _savedInventory,
					savedLevelStats: _savedLevelStats,
					snapshotToLoad: _loadSnapshot);
			...
			if (!_displayMode.IsVRActive)
			{
				...
				_displayMode.GetGripHandNode(1) is Node3D leftGripNode)
			{
	...
	private WeaponFirePose? GetWeaponFirePose(int slotIndex)
	{
		Vector3 forward = _displayMode.IsVRActive
			...
			origin = _displayMode.IsVRActive
	...
	private int? PerformWeaponRaycast(int slotIndex)
	{
		...
		Vector3 cameraForward = -_displayMode.Camera.GlobalTransform.Basis.Z,
			rayOrigin = _displayMode.IsVRActive
				? _displayMode.GetHandPosition(slotIndex)
				: _displayMode.Camera.GlobalPosition,
			rayDirection = _displayMode.IsVRActive
				? _displayMode.GetHandForward(slotIndex)
				: cameraForward;
	...
	private void CycleWeapon(int handIndex, int direction)
	{
		...
		int currentNumber = _simulatorController.Simulator.Inventory.GetValue($"SelectedWeapon{handIndex}"),
			currentIdx = weaponNumbers.IndexOf(currentNumber);
		if (currentIdx < 0)
			currentIdx = 0;
	...
	private void ExecuteTeleport((ushort X, ushort Z) destTile)
	{
		float destWorldX = destTile.X.ToMetersCentered(),
			destWorldZ = destTile.Z.ToMetersCentered();
		...
		Vector3 hmdPos = _displayMode.ViewerPosition,
			originPos = _displayMode.Origin.GlobalPosition;
		...
		_simulatorController.TeleportPlayer(
			playerX: destWorldX.ToFixedPoint(),
			playerZ: destWorldZ.ToFixedPoint(),
			playerAngle: currentAngle);
	...
	private void UseObjectHandIsFacing(int handIndex)
	{
		...
		Vector3 handForward = _displayMode.GetHandForward(handIndex),
			horizontal = new(handForward.X, 0f, handForward.Z);
	...
	private bool IsElevatorAtTile(ushort tileX, ushort tileY) =>
		MapAnalysis.Elevators.Any(elev => elev.X == tileX && elev.Y == tileY);
	/// <summary>
		...
		if (_teleportOverlay is not null)
		{
			if (_displayMode.IsTeleportModeActive)
			{
				Vector3 controllerPos = _displayMode.GetHandPosition(0),// Right controller
					controllerForward = _displayMode.GetHandForward(0),
					viewerPos = _displayMode.ViewerPosition;
				ushort playerTileX = (ushort)Mathf.Max(0, viewerPos.X.ToTile()),
					playerTileZ = (ushort)Mathf.Max(0, viewerPos.Z.ToTile());
	...
	{
		if (SharedAssetManager.StatusBar?.FizzleFadeColor is byte fizzleColorIndex)
			// WL_GAME.C:Died: VW_Bar fills view with the configured palette color, then FizzleFade
			_deathFizzleOverlay?.TriggerFizzle(SharedAssetManager.GetPaletteColor(fizzleColorIndex));
		else
	...
	/// </summary>
	private void OnDeathFizzleComplete() => PendingDeathFadeOut = true;
	/// <summary>
	...
	private void OnVictoryStarted(VictoryStartedEvent e)
	{
		float viewWorldX = e.ViewTileX.ToMetersCentered(),
			viewWorldZ = e.ViewTileY.ToMetersCentered();
		...
		else if (_displayMode.Camera is not null)
		...
		float tileMinX = e.ViewTileX.ToMeters() + Constants.HeadXZ,
			tileMaxX = tileMinX + Constants.TileWidth - Constants.HeadXZ * 2f,
			tileMinZ = e.ViewTileY.ToMeters() + Constants.HeadXZ,
			tileMaxZ = tileMinZ + Constants.TileWidth - Constants.HeadXZ * 2f;
